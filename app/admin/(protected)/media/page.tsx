...
export default async function MediaPage({ searchParams }: { searchParams: Promise<{ deleted?: string; error?: string }> }) {
  ...
  return <>
    ...
    <Card className="mb-8"><CardHeader><div className="flex flex-wrap items-start justify-between gap-4"><div><CardTitle>上传新图片</CardTitle><CardDescription className="mt-1.5">Vercel Blob 和 R2 模式均从浏览器直传，不经过网站服务器。</CardDescription></div>{storageMode ? <Badge variant={storageMode === "LOCAL" ? "warning" : "success"}>{storageMode === "BLOB" ? "Vercel Blob" : storageMode === "R2" ? "Cloudflare R2" : "本地 Storage"}</Badge> : <Badge variant="warning">未配置 Storage</Badge>}</div></CardHeader><CardContent>
      {!storageMode ? <div className="mb-4 rounded-xl border border-amber-200 bg-amber-50 px-4 py-3 text-sm leading-6 text-amber-800">生产环境需要配置 Vercel Blob 或 Cloudflare R2；本地 Storage 仅适用于开发或自托管服务器。</div> : null}
    ...
    {media.length ? <section className="grid gap-5 sm:grid-cols-2 xl:grid-cols-4">{media.map((item) => {
      ...
      return <Card key={item.id} className="overflow-hidden"><div className="aspect-square bg-slate-100"><img src={item.url} alt={item.alt || item.name} loading="lazy" className="size-full object-cover" /></div><CardContent className="p-4">
        <div className="flex items-start justify-between gap-2"><strong className="min-w-0 truncate text-sm" title={item.name}>{item.name}</strong><Badge variant={item.storageProvider === "LOCAL" ? "neutral" : "success"}>{item.storageProvider}</Badge></div>
